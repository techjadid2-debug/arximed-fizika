  ...
  return (
    <main className="min-h-[calc(100dvh-4.5rem)] overflow-hidden">
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[.85fr_1.15fr]">
        <section className="flex min-h-[52svh] flex-col justify-between px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
          <div>
            <p className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[.16em] text-primary">
              <Gauge className="size-4" />
              {t[4]}
            </p>
            <h1 className="mt-8 max-w-xl text-5xl font-semibold leading-[.98] tracking-[-.065em] sm:text-6xl">
              {t[0]}
            </h1>
            <p className="mt-7 max-w-md text-base leading-7 text-muted-foreground">{t[1]}</p>
            <div className="mt-9 flex flex-wrap gap-3">
              <Button asChild className="min-h-12 rounded-md px-6">
                <Link href={`/${locale}/learn/ilk-qadam/01`}>
                  {t[2]}
                  <ArrowRight />
                </Link>
              </Button>
              <Button asChild variant="outline" className="min-h-12 rounded-md px-6">
                <Link href={`/${locale}/courses`}>{t[3]}</Link>
              </Button>
            </div>
          </div>
        ...
        <section className="relative min-h-[52svh] overflow-hidden border-y border-border bg-card lg:border-l">
          <PhysicsCanvas2D />
          <SimulationTelemetry locale={locale} />
          <SimulationControls />
        </section>
      </div>
    </main>
